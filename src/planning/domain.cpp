...
namespace planning {
    ...
    Domain::Domain(const std::string &name,
                   ...
                   const std::vector<Function> &functions,
                   const std::vector<Object> &constant_objects)
    : Domain(name, predicates, functions, constant_objects, std::vector<ActionSchema>()) {}
    ...
    std::unordered_map<std::string, ActionSchema> Domain::get_name_to_action_schema() const {
      std::unordered_map<std::string, ActionSchema> name_to_action_schema;
      for (const auto &action_schema : action_schemas) {
        name_to_action_schema[action_schema.name] = action_schema;
      }
      return name_to_action_schema;
    }
  ...
  int Domain::get_max_arity() const {
    ...
    {
      max_arity = std::max(max_arity, action_schemas[i].arity);
    }
  ...
  json Domain::to_json() const {
    ...
    std::vector<std::pair<std::string, int>> action_schemas_raw;
    for (size_t i = 0; i < predicates.size(); i++) {
    ...
    for (size_t i = 0; i < action_schemas.size(); i++)
    {
      action_schemas_raw.push_back(std::make_pair(action_schemas[i].name, action_schemas[i].arity));
    }

    j["predicates"] = predicates_raw;
    j["functions"] = functions_raw;
    j["action_schemas"] = action_schemas_raw;
  ...
  std::string Domain::to_string() const {
    ...
    repr += "], action_schemas=[";
    for (size_t i = 0; i < action_schemas.size(); i++) {
      repr += action_schemas[i].to_string();
      if (i < action_schemas.size() - 1) {
        repr += ", ";
      }
    }
